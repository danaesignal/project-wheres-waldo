...
class PhotoTagger extends PureComponent{
  ...
  async componentDidMount(){
    const turtleResponse = await fetch(`http://0.0.0.0:4500/target/${this.state.userSelection}`);
    const turtleJson = await turtleResponse.json();
    const turtleData = {...turtleJson[0]};

    const scoreResponse = await fetch(`http://0.0.0.0:4500/score/`);
  ...
  menuClickHandler = async (turtle) => {
    ...
    const turtleResponse = await fetch(`http://0.0.0.0:4500/target/${turtle}`);
    const turtleJson = await turtleResponse.json();
    const turtleData = {...turtleJson[0]};
  ...
  postGameScore = async (scoreCard) => {
    const rawResponse = await fetch('http://0.0.0.0:4500/score/add', {
      method: 'POST',
      headers: {
  ...
  handleNameSubmission = async () => {
    return await fetch(`http://0.0.0.0:4500/score/${this.state.scoreCard.id}`, {
      method: 'PATCH',
      headers: {
        'Accept': 'application/json',
        'Content-Type': 'application/json',
